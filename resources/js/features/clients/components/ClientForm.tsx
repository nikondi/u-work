...
export function ClientForm({id}) {
  ...
  const onSave: FormEventHandler = (e) => {
    ...
    const client_data: ClientRaw = {...client, password: '', address_id: client.address.id, status: statusToNumber[client.status]};
      ...
      <form onSubmit={onSave}>
        <div className="relative">
          ...
          <FormRow label="ФИО" className="mt-4">
            <Input label="ФИО" value={client.name} setValue={(v) => setClient({...client, name: v})}/>
          </FormRow>
          <div className="flex flex-wrap -mx-2">
            <FormRow label="Номер лицевого счёта" className="mt-4 md:w-1/2 px-2">
              <Input label="Номер лицевого счёта" value={client.id} setValue={(v) => setClient({...client, id: getInputInt(v)})}/>
            </FormRow>
          ...
          <FormRow label="Адрес" className="mt-4">
            <Select value={client.address} onChange={v => setClient({...client, address: v})}>
          ...
          <div className="flex flex-wrap -mx-2">
            <FormRow label="Квартира" className="mt-4 md:w-1/2 px-2">
              <Input label="Квартира" type="number" value={client.apartment || ''} setValue={(v) => setClient({...client, apartment: getInputInt(v)})}/>
            </FormRow>
            <FormRow label="Этаж" className="mt-4 md:w-1/2 px-2">
              <Input label="Этаж" type="number" value={client.floor || ''} setValue={(v) => setClient({...client, floor: getInputInt(v)})}/>
            </FormRow>
          </div>
