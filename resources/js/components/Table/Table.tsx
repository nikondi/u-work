...
import {Pagination, ResourceConfig, useResource} from "@/hooks/useResource";
...
const default_tableConfig: TableConfig = {
  columns: [], content: [], primaryKey: null, linkTo: null, buttons: [], rowModules: [], onClick: () => {
  }
};
  ...
  return (<>
        ...
        <table className="w-full text-left mt-5">
          ...
          {sortedFilteredContent.length > 0 ?
              ...
              : <tr>
                <td colSpan={tableConfig.columns.length} className="px-5 py-3 text-center">Пусто...</td>
              </tr>
          }
          {children}
          </tbody>
...
function Row({item}) {
  ...
  return <tr className={"tbl-row relative " + tableConfig.rowClass}>
    {tableConfig.columns.map((column: ColumnConfig) => tableConfig.linkTo && column.linked
        ? <td key={column.key}>
          <Link className="tbl-column" to={tableConfig.linkTo(row)}>{column.filter(row[column.key] ? row[column.key] : '')}</Link>
        </td>
        :
...
type TableServerProps = {
  ...
  setPageRef?: MutableRefObject<(page: number, silent: boolean) => void>
};

const default_tableServer_config = {pagination: false} as Partial<TableServerConfig>;

export function TableServer({config, className = '', setLoading, setPageRef, children}: PropsWithChildren<TableServerProps>) {
  const [sort, _setSort] = useState<Sort>(null);

  config = {...default_tableServer_config, ...config};
  config.tableConfig = {...default_tableConfig, ...config.tableConfig};

  const {list: rows, pagination, loading: resourceLoading, setPage}: any = useResource({
...
function SortButtons({sort, setSort, column}) {
  return <div className="sort text-gray-500">
    <button className={'sort-asc' + (sort?.column.key === column.key && sort.order === 'asc' ? ' active' : '')} onClick={() => setSort({column: column, order: 'asc'})}>
      <svg width="10" style={{transform: 'rotate(180deg)'}} viewBox="0 0 123.959 123.958">
        <path d="M117.979 28.017h-112c-5.3 0-8 6.4-4.2 10.2l56 56c2.3 2.3 6.1 2.3 8.401 0l56-56c3.799-3.8 1.099-10.2-4.201-10.2z" fill="currentColor"></path>
      </svg>
    </button>
    <button className={'sort-desc' + (sort?.column.key === column.key && sort.order === 'desc' ? ' active' : '')} onClick={() => setSort({column: column, order: 'desc'})}>
      <svg width="10" viewBox="0 0 123.959 123.958">
        <path d="M117.979 28.017h-112c-5.3 0-8 6.4-4.2 10.2l56 56c2.3 2.3 6.1 2.3 8.401 0l56-56c3.799-3.8 1.099-10.2-4.201-10.2z" fill="currentColor"></path>
      </svg>
    </button>
  </div>
